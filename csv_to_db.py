...
def viewDatabaseTable(database, tblName):
    conn = sqlite3.connect(database)
    curs = conn.cursor()
    curs.execute("SELECT * FROM tblPhos")
    rows = curs.fetchall()
    for row in rows:
        print(row)
...
if __name__ == '__main__':
    createSqliteConnection(r"data/phosphorus_database.db")
    pandasToDatabase("data/phosphorusFood.csv", "data/phosphorus_database.db", "tblPhos", )
    viewDatabaseTable("data/phosphorus_database.db", "tblPhos")
